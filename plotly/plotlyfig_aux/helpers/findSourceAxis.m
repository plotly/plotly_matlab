function [xsource, ysource, xoverlay, yoverlay] = findSourceAxis(obj, axIndex, yaxIndex)
    ...
    if overlapping
        if isequal(get(obj.State.Axis(axIndex).Handle, 'XAxisLocation'), ...
                    get(obj.State.Axis(overlapaxes(1)).Handle, 'XAxisLocation'))
            xsource = overlapaxes(1);
        else
            xoverlay = overlapaxes(1);
        end
        if isequal(get(obj.State.Axis(axIndex).Handle, 'YAxisLocation'), ...
                    get(obj.State.Axis(overlapaxes(1)).Handle, 'YAxisLocation'))
