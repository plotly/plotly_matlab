...
% update axis handle
obj.State.Axis.Handle = text_data.Parent;
...
%-AXIS XLABEL-%
if eq(event.AffectedObject,get(obj.State.Axis.Handle,'XLabel'))
    
    %     switch prop
    %         case {'String','Interpreter'}
    %             %-title-%
    %             if ~isempty(text_data.String)
    %                 obj.layout.xaxis{obj.getCurrentAxisIndex}.title = parseString(text_data.String,text_data.Interpreter);
    %             end
    %         case 'Color'
    %             %-title font color-%
    %             col = 255*text_data.Color;
    %             obj.layout.xaxis{obj.getCurrentAxisIndex}.titlefont.color = ['rgb(' num2str(col(1)) ',' num2str(col(2)) ',' num2str(col(3)) ')'];
    %         case 'FontSize'
    %             %-title font size-%
    %             obj.layout.xaxis{obj.getCurrentAxisIndex}.titlefont.size = text_data.FontSize;
    %         case 'FontName'
    %             %-title font family-%
    %             obj.layout.xaxis{obj.getCurrentAxisIndex}.titlefont.family = matlab2plotlyfont(text_data.FontName);
    %     end
